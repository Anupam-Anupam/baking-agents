...
TRAIN_BASE_MODEL = getattr(C, "BASE_MODEL_PATH", C.MODEL_NAME)
WANDB_PROJECT = os.getenv("WANDB_PROJECT", C.WANDB_PROJECT)
WANDB_ENTITY = os.getenv("WANDB_ENTITY") or None
WANDB_RUN_GROUP = os.getenv("WANDB_RUN_GROUP") or None
WANDB_RUN_NAME = os.getenv("WANDB_RUN_NAME") or f"window-bake-topk{C.TOP_K}"
WANDB_JOB_TYPE = os.getenv("WANDB_JOB_TYPE", "window_bake")
WANDB_ORCHESTRATOR_RUN_ID = os.getenv("WANDB_ORCHESTRATOR_RUN_ID", "")
WANDB_WINDOW_INDEX = int(os.getenv("WANDB_WINDOW_INDEX", "0") or 0)
WANDB_MODE = os.getenv("WANDB_MODE", "").strip().lower()
WANDB_DISABLED = WANDB_MODE == "disabled" or os.getenv("WANDB_DISABLED", "").strip().lower() in {"1", "true", "yes"}
TRAIN_METRICS_OUTPUT = os.getenv("TINKER_TRAIN_METRICS_PATH", "").strip()


# Convert a train-state checkpoint URI to its sampler checkpoint URI.
def _sampler_path_from_state_path(path: str) -> str:
    if "/weights/" in path:
        return path.replace("/weights/", "/sampler_weights/")
    return path
...
def train():
    ...
    if TRAIN_BASE_MODEL.startswith("tinker://"):
        # Continue training from a previous baked checkpoint state.
        tc = service.create_training_client_from_state(TRAIN_BASE_MODEL)
        prompted_sc = service.create_sampling_client(
            model_path=_sampler_path_from_state_path(TRAIN_BASE_MODEL)
        )
    else:
        tc = service.create_lora_training_client(base_model=TRAIN_BASE_MODEL, rank=C.LORA_RANK)
        prompted_sc = service.create_sampling_client(base_model=TRAIN_BASE_MODEL)
    ...
    wandb.init(
        ...
        config={
            ...
            "resume_from_state": TRAIN_BASE_MODEL.startswith("tinker://"),
            "window_index": WANDB_WINDOW_INDEX,
            "orchestrator_run_id": WANDB_ORCHESTRATOR_RUN_ID,
    ...
    train_metrics: dict[str, float] = {
        "steps": 0.0,
        "epochs": float(C.NUM_EPOCHS),
        "last_avg_kl_per_token": 0.0,
        "last_kl_loss": 0.0,
        "last_lr": 0.0,
        "last_datums": 0.0,
    }
    ...
    try:
        for epoch in range(C.NUM_EPOCHS):
            for batch_idx in range(n_batches_per_epoch):
                ...
                if fb_result.metrics:
                    ...
                    train_metrics["last_avg_kl_per_token"] = float(fb_result.metrics.get("avg_kl_per_token", 0.0))
                    train_metrics["last_kl_loss"] = float(fb_result.metrics.get("kl_loss", 0.0))
                train_metrics["steps"] = float(step)
                train_metrics["last_lr"] = float(C.LEARNING_RATE * lr_mult)
                train_metrics["last_datums"] = float(len(all_datums))
        ...
        try:
            ...
            wandb.summary["window_index"] = WANDB_WINDOW_INDEX
            wandb.summary["orchestrator_run_id"] = WANDB_ORCHESTRATOR_RUN_ID
        except Exception as e:
            logger.error(f"Failed to save final checkpoint: {e}")
        if TRAIN_METRICS_OUTPUT:
            try:
                Path(TRAIN_METRICS_OUTPUT).write_text(json.dumps(train_metrics, indent=2), encoding="utf-8")
            except Exception as e:
                logger.error(f"Failed to write train metrics output: {e}")
        wandb.finish()

    return tc, train_metrics
...
def main():
    ...
    print(f"Train Base: {TRAIN_BASE_MODEL}")
