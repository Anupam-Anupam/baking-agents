...
class BreadClient:
    """
    Thin wrapper over the Bread SDK with dry-run mode.

    This keeps Bread-specific calls isolated from the rest of the pipeline.
    """
    ...
    def repo_set(self, repo_name: str, base_model: str | None = None) -> dict[str, Any]:
        ...
        response = self._client.repo.set(repo_name=repo_name, base_model=base_model)
        return {"repo_name": response.repo_name, "base_model": response.base_model}
    ...
    def target_set(
        self, repo_name: str, target_name: str, teacher_prompt_name: str, student_prompt_name: str, generators: list[dict[str, Any]]
    ) -> dict[str, Any]:
        if self.dry_run:
    ...
    def run_stim(self, repo_name: str, target_name: str) -> dict[str, Any]:
        if self.dry_run:
            return {"status": "complete", "lines": 0, "dry_run": True}
